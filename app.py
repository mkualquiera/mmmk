...
async def websocket_endpoint():
    ...
    if msg_data["type"] != "meta":
        await websocket.send(
            json_util.dumps(
                {"type": "posted", "error": "Expected a message of type meta"}
            )
        )
        return
    ...
    if not nickname:
        ...
        if nickname_data["type"] != "nickname":
            await websocket.send(
                json_util.dumps(
                    {"type": "posted", "error": "Expected a message of type nickname"}
                )
            )
            return
    ...
    if signature_data["type"] != "signature":
        await websocket.send(
            json_util.dumps(
                {"type": "posted", "error": "Expected a message of type signature"}
            )
        )
        return
    ...
    recovered = w3.eth.account.recover_message(encoded_text, signature=signature)[
        2:
    ].lower()
    ...
    if recovered == address:
        ...
        await websocket.send(
            json_util.dumps(
                {"type": "posted", "error": "Unable to verify the message signature"}
            )
        )
